...
describe('FeedTabsComponent', () => {
  ...
  it('should display one tab by default', () => {
    ...
    const tabLinks = fixture.debugElement.queryAll(By.css('[data-test=feed-tab-link]'));
  ...
  it('should display three tabs if the user is logged in and a tag is selected', () => {
    ...
    expect(tabLinks[0].nativeElement.textContent).toContain('Global Feed');
    expect(tabLinks[1].nativeElement.textContent).toContain('Your Feed');
    expect(tabLinks[2].nativeElement.textContent).toContain('#dragons');
  });
  ...
  it('should make the first tab active when displaying the global feed', () => {
    ...
    const tabLinks = fixture.debugElement.queryAll(By.css('[data-test=feed-tab-link]'));
    expect(tabLinks.length).withContext('You should have two links').toBe(2);
  ...
  it('should make the third tab active when displaying the feed based on the selected tag', () => {
    ...
    const tabLinks = fixture.debugElement.queryAll(By.css('[data-test=feed-tab-link]'));
    expect(tabLinks.length).withContext('You should have three links').toBe(3);
    expect(tabLinks[0].nativeElement.className)
      .withContext('The first link should NOT be active')
      .not.toContain('active');
    expect(tabLinks[1].nativeElement.className)
      .withContext('The second link should NOT be active')
      .not.toContain('active');
    expect(tabLinks[2].nativeElement.className)
  ...
  it('should emit an output event on feed change', () => {
    ...
    tabLinks[1].triggerEventHandler('click');
    ...
    tabLinks[0].triggerEventHandler('click');
    expect(component.changed.emit)
      .withContext('You should have the click handler on the `a` element')
